...
// public/audio-cache 配下に置き、それが存在する場合はライブ生成(Azure)を
...
// contrastSetId: 練習文のID、gender: "male" | "female"
function cachedAudioPath({ kind, contrastSetId, gender }) {
  const safeKind = sanitizeSegment(kind);
  const safeSetId = sanitizeSegment(contrastSetId);
  const safeGender = sanitizeSegment(gender);
  if (!safeKind || !safeSetId || !safeGender) return null;
  return path.join(CACHE_ROOT, safeKind, safeSetId, `${safeGender}.mp3`);
}
