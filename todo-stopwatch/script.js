(async () => {
  ...
  function migratePriorityTasksToItems() {
    ...
    // 他のsave*()と違い、このキーは書き込むための専用ラッパーを持たない
    // ままここでbareにremoveItemしていたため、クラウド側の(移行前の)古い
    // 値がいつまでも残り続けていた — サインイン中の別端末がリロードする
    // たびにその古い値を「まだ移行されていないデータ」として再度pullして
    // きて、ここでもう一度itemへ変換・追記してしまい、最優先タスクが
    // リロードのたびに増殖する不具合の原因だった。ここでも明示的に空を
    // 同期しておくことで、クラウド側も空に収束させる。
    window.AppSync?.markDirty("priorityTasks:v1", {});
  }
  ...
  // 用紙の向き(縦/横)をCSSの@pageで指定する。iOS Safariの印刷シートは
  // 既定で「縦向き」を選んだ状態になり、CSS側の指定だけでは自動的に
  // 「横向き」を選ばせられないことがある(プラットフォーム側の制約で、
  // ここは利用者に手動で切り替えてもらう必要がある)。せめてどちらの
  // 向きを選んでも中身が正しく表示されるよう、#printAreaの表示切り替え
  // はJSが付け外しするクラスに頼らず@media print自体で常に行う
  // (style.css参照) — 以前はbody.print-modeというクラスに依存して
  // いたが、iOSで印刷シート内の向きを手動で切り替えるとプレビューが
  // 再計算され、その時点でクラスが残っている保証がないため、真っ白な
  // ページになってしまう不具合があった。
  function triggerPrint(orientation) {
    ...
    printPageStyle.textContent = `@page { size: ${size}; margin: ${margin}; }`;
    window.print();
  }
  ...
  function planChildrenAtDepth(plan, depth) {
    if (depth <= 1) return planChildrenOf(plan, null);
    const parents = planChildrenAtDepth(plan, depth - 1);
    const result = [];
    parents.forEach((p) => result.push(...planChildrenOf(plan, p.id)));
    return result;
  }

  // 子を持つノード(=線を引く必要がある箇所)を、そのノードと直接の子の
  // 組として全て集める。木全体が常に全展開なので、いつかの
  // activeSomedayIdsのような「今どれを掘り下げているか」の状態は不要。
  function planConnectorPairs(plan) {
    const pairs = [];
    (plan.children || []).forEach((node) => {
      const kids = planChildrenOf(plan, node.id);
      if (kids.length) pairs.push({ parent: node, children: kids });
    });
    return pairs;
  }

  // タスクページ(いつか一覧)の縦一覧と同じ見た目/操作感: 子を持つノードは
  // 塗りつぶし角丸チップ+子数バッジで見分けられる。木は常に全展開なので、
  // タップすると(掘り下げではなく)直接、変更修正/子タスク追加/完了/削除
  // の選択肢パネルが開く — いつかのopenTaskChoiceModalをそのまま再利用。
  function createPlanChildChip(dateStr, plan, child, depth) {
    const chip = document.createElement("button");
    chip.type = "button";
    chip.className = "cal-unplanned-chip";
    chip.dataset.planChildId = child.id;
    ...
    chip.addEventListener("click", () => promptPlanChildEditOrAddChild(dateStr, plan, child, depth));
    return chip;
  }

  function buildPlanChildColumn(dateStr, plan, title, depth) {
    const nodes = planChildrenAtDepth(plan, depth);
    if (!nodes.length && depth > 1) return null;

    const col = document.createElement("div");
    col.className = "cal-plan-child-col";
    const header = document.createElement("div");
    header.className = "cal-plan-child-col-header";
    const titleEl = document.createElement("span");
    ...
    list.className = "cal-plan-child-col-list";
    nodes.forEach((child) => list.appendChild(createPlanChildChip(dateStr, plan, child, depth)));
    col.appendChild(list);
    return col;
  }

  // 木全体を横に並んだ「子タスク/孫タスク/ひ孫タスク」の列として常に
  // 全展開表示する(タスクページのMiller Columnsと同じ向き)。縦幅に収まら
  // ない分はこの予定詳細パネル自体(calendarDetail)のスクロールに任せる。
  function buildPlanChildrenSection(dateStr, plan) {
    const wrap = document.createElement("div");
    wrap.className = "cal-plan-children";

    const col1 = buildPlanChildColumn(dateStr, plan, "子タスク", 1);
    wrap.appendChild(col1);
    const col2 = buildPlanChildColumn(dateStr, plan, "孫タスク", 2);
    if (col2) wrap.appendChild(col2);
    const col3 = buildPlanChildColumn(dateStr, plan, "ひ孫タスク", 3);
    if (col3) wrap.appendChild(col3);

    requestAnimationFrame(() => drawPlanChildConnectorLines(wrap, plan));
    return wrap;
  }

  // 列をまたぐ親子の連結線を描く。幾何はpositionSomedayConnectorHorizontal
  // と同じ(親チップ右端から幹→縦の梁→各子チップへ横の枝)だが、木は常に
  // 全展開なので「今アクティブな1本」ではなく、子を持つノード全てについて
  // 描く(drawSomedayTreeConnectorLinesの複数ペア対応と同じ考え方)。
  function drawPlanChildConnectorLines(containerEl, plan) {
    containerEl.querySelectorAll(".someday-branch-line").forEach((el) => el.remove());
    const pairs = planConnectorPairs(plan);
    if (!pairs.length) return;
    const rootRect = containerEl.getBoundingClientRect();
    const half = SOMEDAY_BRANCH_LINE_W / 2;

    pairs.forEach(({ parent, children }) => {
      const parentChip = containerEl.querySelector(`[data-plan-child-id="${parent.id}"]`);
      const childChips = children.map((c) => containerEl.querySelector(`[data-plan-child-id="${c.id}"]`)).filter(Boolean);
      ...
      const stemLeft = parentRect.right - rootRect.left;
  ...
  function renderSomedayList() {
    // ウィークリー/デイリーだけ、全展開ツリー(.someday-tree)が欄本体
    // (.calendar-unplanned)の中に直接入っている(今週中と同じ配置)ため、
    // 今週中と同じくボックス自体にもgroupedを付けて align-items を
    // flex-start に切り替える — 付けないと「たたむ」「＋」ボタンが
    // ツリーの高さ全体の中央に来てしまい、ツリーが縦に伸びるほど
    // 今週中の配置(常に上端)とズレていく。
    [dailyUnplannedBoxEl, weeklyUnplannedBoxEl].forEach((box) => box.classList.toggle("grouped", somedayExpandAll));
    if (somedayExpandAll) {
    ...
    // stale chip position on THIS synchronous measurement, even though
    // getBoundingClientRect is normally accurate immediately — re-measuring
    ...
    // タスクページ全体(最優先/スケジュール内/今日中/今週中/いつかを
    // カテゴリ見出し無しで時系列順に1本の縦並びにしたもの)は、他ページの
    // 全展開トグルの状態にもactiveSomedayIds(1つの経路だけ掘り下げる通常
    // モードの状態)にも関知しない。renderSomedayList()はrenderCalendar()
    // を経由せず単独で呼ばれる箇所が多い(子タスク追加直後など)ため、
    // renderCalendar()側の別呼び出しに頼るとその経路だけタスクページが
    // 古いまま(または一部の欄がactiveSomedayIdsに引きずられて隠れたまま)
    // になる。ここで毎回呼んで確実に同期させる。
    renderTasklistUnifiedList();
  }
  ...
  // 子タスク追加のような掘り下げは無いが、チップの操作自体はいつかと同じ
  ...
  // handleSomedayChipTap(depth===0の無関係な葉タスクをタップした場合の
  ...
  // 今週中チップの「実質タップ」を一元的に処理する — 子タスクを抱えた
  // チップなら展開/折りたたみの切り替え、そうでなければ
  // handleThisWeekLeafTap。指の揺れでpendingのまま最後まで留まらず
  ...
  // lineHostElは実際に線(.someday-branch-line)を追加する場所・座標の
  // 基準(position:relativeであること)、scopeElはチップ検索の範囲
  // (THISWEEK_ROOT_NODE_IDは複数の家族で使い回すため、家族ごとに範囲を
  // 絞らないと最初に見つかった無関係なチップに繋がってしまう)。通常の
  // ドリルダウン中は専用行(親チップは別の行にあるので両方を含む
  // .calendar/.pageまるごと)がその両方を兼ね、全展開のツリー表示中は
  // 家族ごとのdisplay:contentsラッパー(位置は持たないのでscopeElのみに
  // 使う)と、実際に箱を持つツリー全体(lineHostEl/rootEl)とで役割を
  // 分ける。
  // 呼び出し側で(複数回呼ばれる場合は最初の1回だけ)lineHostElの既存の
  // 線を消してから呼ぶこと — ここでは消さない(全展開ツリーは家族ごとに
  // 別々に呼ばれるため、ここで毎回消すと前の家族の線を消してしまう)。
  function drawThisWeekConnectorLines(lineHostEl, rootEl, scopeEl, pairs) {
    if (!pairs.length) return;
    const rootRect = rootEl.getBoundingClientRect();
    const half = SOMEDAY_BRANCH_LINE_W / 2;
    pairs.forEach(({ parentId, children }) => {
      const parentChip = scopeEl.querySelector(`[data-thisweek-node-id="${parentId}"]`);
      const childChips = children.map((c) => scopeEl.querySelector(`[data-thisweek-node-id="${c.id}"]`)).filter(Boolean);
      if (!parentChip || !childChips.length) return;
      const parentRect = parentChip.getBoundingClientRect();
      const childRects = childChips.map((el) => el.getBoundingClientRect());
  ...
  // 全展開表示: いつかの全展開ツリー(renderSomedayTreeInto/isVertical=
  // false)と同じ、CSS Gridで「列=アイテム・行=深さ」に並べる方式。
  // 各アイテムは深さ0(自分自身)のセルを必ず持ち、子孫がいる分だけ
  // 深さ1〜3のセルを追加する。家族(1アイテム分のセル一式)は
  // display:contentsのラッパーで束ね、グリッドのレイアウトには影響
  // させずに連結線描画の検索範囲としてだけ使う(THISWEEK_ROOT_NODE_ID
  // は全家族で使い回す値なので、範囲を絞らないと最初に見つかった
  // 無関係なチップに繋がってしまうため)。
  function buildThisWeekTreeInto(listEl, items, weekStart) {
    listEl.innerHTML = "";
    if (!items.length) {
      const empty = document.createElement("span");
      empty.className = "calendar-unplanned-empty";
    ...
    // 子を持つタスクを先に並べる(いつかのsortSomedayChildrenForTreeと
    // 同じ考え方) — 元の相対順は保つ安定ソート。
    const ordered = [...items].sort((a, b) => {
      const bHas = b.children && b.children.length ? 1 : 0;
      const aHas = a.children && a.children.length ? 1 : 0;
      return bHas - aHas;
    });
    const families = [];
    ordered.forEach((item, familyIndex) => {
      const familyWrap = document.createElement("div");
      familyWrap.className = "cal-thisweek-tree-family";
      listEl.appendChild(familyWrap);

      const rootCell = document.createElement("div");
      rootCell.className = "cal-thisweek-tree-cell";
      rootCell.style.gridColumn = String(familyIndex + 1);
      rootCell.style.gridRow = "1";
      familyWrap.appendChild(rootCell);
      rootCell.appendChild(createThisWeekChip(item, weekStart, true));

      for (let depth = 1; depth <= PLAN_CHILD_MAX_DEPTH; depth++) {
        const nodes = planChildrenAtDepth(item, depth);
        if (!nodes.length) break;
        const cell = document.createElement("div");
        cell.className = "cal-thisweek-tree-cell";
        cell.style.gridColumn = String(familyIndex + 1);
        cell.style.gridRow = String(depth + 1);
        familyWrap.appendChild(cell);
        nodes.forEach((child) => cell.appendChild(createThisWeekChildChip(child, item)));
      }
      families.push({ familyWrap, item });
    });
    // レイアウト確定後(チップの実際の位置が定まった1フレーム後)に
    // 連結線を引く — buildPlanChildrenSectionと同じ考え方。
    requestAnimationFrame(() => {
      listEl.querySelectorAll(".someday-branch-line").forEach((el) => el.remove());
      families.forEach(({ familyWrap, item }) => {
        drawThisWeekConnectorLines(listEl, listEl, familyWrap, thisWeekConnectorPairs(item));
      });
    });
  }
  ...
  function renderThisWeekTray() {
    ...
    // いつかの全展開ツリーと同じ「列=アイテム・行=深さ」のグリッド
    // 表示へ切り替える。通常のドリルダウン(1件だけタップして展開)
    // では、いつか同様、上の行は常に横並びの一覧のまま変えず、子孫は
    // 専用の別行に表示する。
    ...
    [weeklyThisWeekList, dailyThisWeekList].forEach((list) => {
      ...
      if (thisWeekExpandAll) {
        buildThisWeekTreeInto(list, items, weekStart);
        return;
      }
      list.innerHTML = "";
      if (!items.length) {
    ...
    requestAnimationFrame(() => {
      [weeklyThisWeekDrilldownEl, dailyThisWeekDrilldownEl].forEach((drilldownEl) => {
        drilldownEl.querySelectorAll(".someday-branch-line").forEach((el) => el.remove());
        const rootEl = drilldownEl.closest(".calendar, .page");
        if (rootEl) drawThisWeekConnectorLines(drilldownEl, rootEl, rootEl, thisWeekConnectorPairs(activeThisWeekItem));
      });
    });
  }
  ...
  function onTasklistChipDragMove(e) {
    ...
    if (ctx.phase === "pending") {
      ...
      if (!ctx.heldLongEnough) {
        ...
        // ここでもJS側でscrollElのscrollTopを直接動かす。ここでreturnせず
        // 下の"scroll"フェーズの処理へそのまま流し込み、しきい値を越えた
        // このイベント自身の分もすぐ反映する(横トレイのscrollフェーズは
        // 元々そうなっている) — returnしていると最初の一手だけ画面が
        // 動かないまま次のpointermoveを待つことになり、指への追従が
        // 一拍遅れて「反応が悪い」体感の原因になっていた。
  ...
  // ============================================================
  // タスクページ: カテゴリ(最優先/スケジュール内の予定/今日中/今週中/
  // いつか)の見出しを一切出さず、日時が決まっているものから時系列順に
  // 1列の縦並びで並べる。
  //   順序: 今日の最優先→今日のスケジュール内(子タスクを持つ予定、時刻
  //   順)→今日の今日中→明日の最優先→…(その週の残りの曜日を同じ順で)
  //   →その週の今週中タスク→翌週月曜の最優先→…(以下同様、内容がある週
  //   だけ繰り返す)→最後にいつかタスク。
  //   個々のタスクがどのカテゴリ由来かを示すラベルは出さない — 並び順
  //   だけで「上から時系列順に並んでいる」という共通認識を作る。
  // ============================================================

  // 今週中/いつかのように子孫を持ち得るタスクを、長押しドラッグでルートごと
  // (子孫を引き連れたまま)並べ替えられるようにするための入れ物。
  // startTasklistChipDrag自体は「1つの要素を動かす」ことしか知らないので、
  // ルート行+インデントした子孫行をまとめてこのwrapper(cal-unplanned-chip
  // クラスを持ち、listEl直下の並べ替え対象として認識される)に入れ、wrapper
  // ごと動かすことで子孫が置き去りにならないようにする。子孫行自身も
  // cal-unplanned-chipクラスを持つが、listElの直接の子ではなくwrapperの
  // 子なので、並べ替えの対象(listEl.children)には数えられない。
  function createTasklistFamilyWrapper() {
    const wrapper = document.createElement("div");
    wrapper.className = "cal-unplanned-chip tasklist-family-block";
    return wrapper;
  }

  function createTasklistRowChip(labelText, depth, { hasChildren } = {}) {
    const chip = document.createElement("button");
    chip.type = "button";
    chip.className = "cal-unplanned-chip";
    chip.dataset.depth = String(depth);
    if (hasChildren) chip.classList.add("parent");
    const label = document.createElement("span");
    label.className = "cal-unplanned-chip-label";
  ...
  function appendTasklistChildCountBadge(chip, count) {
    const badge = document.createElement("span");
    badge.className = "cal-child-count-badge";
  ...
  // 「タスクの一覧+末尾の＋ボタン」1本を作る(旧来のカテゴリ見出しは無い)。
  function createTasklistList() {
    const list = document.createElement("div");
    list.className = "calendar-unplanned-list tasklist-list";
    return list;
  }

  function appendTasklistAddBtn(list, ariaLabel, onAdd) {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "calendar-someday-add";
  ...
  // 子タスクを持つ予定(スケジュール内)1件ぶんの行(ルート+子/孫/ひ孫を
  // インデントして直後に並べる)。予定はタスクページ上で並べ替え/追加を
  ...
  // その週の今週中タスク一式(emptyOkの週(=今週)だけ、中身が空でもリスト
  // +＋ボタンを必ず出す)。
  function appendTasklistThisWeekCluster(container, weekStart, emptyOk) {
    const items = itemsArrayForWeek(weekStart);
    if (!items.length && !emptyOk) return;
    const list = createTasklistList();
    items.forEach((item) => list.appendChild(buildTasklistThisWeekFamilyBlock(item, weekStart, list)));
    appendTasklistAddBtn(list, "今週中のタスクを追加", () => addThisWeekTask(weekStart));
    container.appendChild(list);
  }

  // いつかタスクの子孫チップ(表示専用、タップで編集モーダルのみ)。色付け
  // は他ページのcreateSomedayChipと揃える(家族ごとにensureParentColorで
  // 色を割り当て、深さに応じてlightenHexでフェード)。
  function createTasklistSomedayDescendantChip(task, depth) {
    const chip = createTasklistRowChip(task.label, depth, { hasChildren: isParentTask(task.id) });
    if (isParentTask(task.id)) {
      const base = ensureParentColor(rootSomedayTask(task));
      chip.style.borderColor = base;
      chip.style.background = lightenHex(base, SOMEDAY_FILL_LIGHTEN[depth] || 0);
  ...
  // いつかタスク一式。並び順全体の一番最後、常に(空でも)リスト+＋を出す
  // (他ページ同様、いつかへ新規追加する入り口は常にあるべきため)。
  function appendTasklistSomedayCluster(container) {
    const tasks = topLevelSomeday();
    const list = createTasklistList();
    tasks.forEach((task) => list.appendChild(buildTasklistSomedayFamilyBlock(task, list)));
    appendTasklistAddBtn(list, "いつかを追加", addSomedayTask);
    container.appendChild(list);
  }

  // 内容がある(今日以降の)週の月曜日を全て集める — 今週(currentWeekStart)
  // は中身が無くても常に含める(今日の最優先/今日中の＋ボタンを出すため)。
  function tasklistRelevantWeekStarts() {
    const currentWeekStart = mondayOfWeek(state.day);
    const weekStarts = new Set([currentWeekStart]);
    Object.keys(drafts).forEach((d) => {
      if (d >= state.day) weekStarts.add(mondayOfWeek(d));
    });
    Object.keys(plans).forEach((d) => {
      if (d >= state.day) weekStarts.add(mondayOfWeek(d));
    });
    Object.keys(thisWeekTasks).forEach((ws) => {
      if (ws >= currentWeekStart && thisWeekTasks[ws] && thisWeekTasks[ws].length) weekStarts.add(ws);
    });
    return Array.from(weekStarts).sort();
  }

  // タスクページ全体をこの順で組み立てる:
  //   [今週、今日から残りの曜日まで、曜日ごとに最優先→スケジュール内→
  //   今日中]→[今週の今週中]→[来週、月曜から日曜まで同じ順]→[来週の
  //   今週中]→…(内容がある週だけ繰り返す)→[いつか]
  // カテゴリの見出しは一切出さない — 並び順そのものが「時系列順」を表す。
  function renderTasklistUnifiedList() {
    tasklistUnifiedList.innerHTML = "";
    const currentWeekStart = mondayOfWeek(state.day);

    tasklistRelevantWeekStarts().forEach((weekStart) => {
  ...
  // いつか同様に階層を許容するため) — スケジュールへ落とす時と同じ方針
  // (buildPlanChildrenFromSomedayTask)。呼び出し側でcaptureUndoSnapshot/
  // vibrate/renderCalendarを行うこと。
  function addSomedayTaskToThisWeek(weekStart, task) {
  ...
  function onTrayItemDragMove(e) {
    ...
    if (ctx.phase === "schedule") {
      ...
      let targetCol = null;
      for (const col of cols) {
        if (col.dataset.date !== ctx.dateStr) continue;
        const rect = col.getBoundingClientRect();
        if (
          e.clientX >= rect.left &&
          e.clientX <= rect.right &&
  ...
  function onTrayItemDragEnd() {
    ...
    // 今週中チップのタップ(子タスクを抱えていれば展開/折りたたみの
    // 切り替え、そうでなければhandleThisWeekLeafTap)は、実機の指の揺れで
    // 「pending」のまま最後まで留まらずschedule/scroll等へ倒れてしまっても
    // 確実に効くようにしたい。距離のしきい値で「これはタップだったはず」
    // と先読みするのではなく、各フェーズが実際に「有効な移動先が無かった/
    // 実質何も起きなかった」と結論した、まさにその地点でだけ扱う(距離に
    // 依存しないぶん、揺れの大小に関係なく確実に効く)。
    const isThisWeekTap = ctx.scopeType === "week";
    ...
    if (ctx.phase === "schedule") {
      ...
      if (scopeType === "week") {
        ...
        if (!targetCol) {
          ...
          // 着地だった。移動量がごく小さい場合だけ、指の揺れで
          // 「pending」のまま留まらずscheduleフェーズへ倒れてしまった
          // だけの実質タップとみなしhandleThisWeekTapLikeReleaseで処理
      ...
      if (overUnplannedBox) {
        if (item.elapsedMs > 0 || item.running) {
          ...
          // いつか entries carry no time fields to hold it
          vibrate([10, 30, 10]);
          return;
        }
        captureUndoSnapshot();
  ...
  function onSomedayChipDragEnd() {
    ...
    if (ctx.phase === "schedule") {
      ...
      if (targetThisWeekBox) {
        addSomedayTaskToThisWeek(currentWeekStartKey(), task);
        vibrate(20);
        renderCalendar();
        return;
      }
  ...
  function renderCalendar() {
    ...
    if (CAL_DAYS === 1) {
      ...
      // Setting .value moves the cursor to the end but leaves scrollTop
      // wherever it last was (browsers don't reset it just because the value
      // changed) — so revisiting a day whose memo overflows the box showed
      // wherever the cursor/scroll happened to be left, not the first line.
      briefingMemoInput.scrollTop = 0;
      briefingMemoInput.setSelectionRange(0, 0);
          ...
          plansForDate(dateStr).map((p) => ({
            ...
            if (p.blockDisplay === "memo" && p.memo) {
              ...
              if (directChildren.length) {
                const extra = document.createElement("div");
                extra.className = "cal-plan-block-extra";
                // メモ表示と見分けやすいよう、子タスクの項目名の頭に「◻︎」を付ける
                extra.textContent = directChildren.map((c) => `◻︎${c.label}`).join(" / ");
                block.appendChild(extra);
              }
            }
    ...
    renderThisWeekTray();
    // renderSomedayList()が末尾でrenderTasklistUnifiedList()も呼ぶので
    // タスクページ全体(最優先/スケジュール内/今日中/今週中/いつか)は
    // ここで自動的に同期される。
    renderSomedayList();
