    ...
    if (ctx.phase === "schedule") {
      ...
      if (scopeType === "week") {
        ...
        if (!targetCol) {
          ...
          // 着地だった。これは指の揺れで「pending」のまま留まらず
          // scheduleフェーズへ倒れてしまっただけの実質タップとみなし、
          // handleThisWeekTapLikeReleaseで処理する(距離のしきい値を
          // 使わず、フェーズが実際に「有効な移動先が無かった」と判定
          // したこの地点でだけ行うので、揺れの大小に関係なく確実に効く)。
