(async () => {
  ...
  const ANNOUNCEMENTS = [
    { date: "2026-09-17", text: "マンスリー/ウィークリー印刷で、用紙の向き(横)がブラウザによっては反映されないことがある問題に対処しました。「A4横向き」という指定の仕方ではなく、横向きの実寸(297mm×210mm)を直接指定するよう変更しています。" },
  ...
  function triggerPrint(orientation) {
    // "size: A4 landscape"というキーワード指定は、ブラウザによっては
    // 印刷ダイアログの用紙の向き(縦/横)に反映されないことがある —
    // 代わりにA4の実寸(297mm×210mm/210mm×297mm)を直接指定する方が、
    // 用紙の向きとして確実に伝わる。
